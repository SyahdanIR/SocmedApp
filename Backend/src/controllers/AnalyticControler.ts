...
export const popularThread = async (req: Request, res: Response) => {
...
export const countData = async (req: Request, res: Response) => {
  ...
  const totalLikes = countLikes.reduce(
    (acc, thread) => acc + thread._count.likes,
    0,
  );
  const totalReply = countLikes.reduce(
    (acc, likes) => acc + likes._count.replies,
    0,
  );
...
export const notifications = async (req: Request, res: Response) => {
  ...
  const notifications = [...likeNotif, ...followNotif, ...replyNotif].sort(
...
export const followerGrowth = async (req: Request, res: Response) => {
  const userId = (req as any).user.id;
  ...
  const grouped = follows.reduce((acc: Record<string, number>, follow) => {
